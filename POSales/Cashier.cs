...
namespace POSales
{
    public partial class Cashier : Form
    {
        ...
        private void btnSettle_Click_1(object sender, EventArgs e)
        {
            slide(btnSettle);
        ...
        private void btnClear_Click_1(object sender, EventArgs e)
        {
            ...
            {
                cn.Open();
                cm = new SqlCommand("Delete from tbCart where trasnno like'" + lblTranNo.Text + "'", cn);
                cm.ExecuteNonQuery();
                cn.Close();
        ...
        public void LoadCart()
        {
            try
            {
                ...
                dgvCash.Rows.Clear();
                cn.Open();
                cm = new SqlCommand("SELECT c.id, c.pcode, p.pdesc, c.price, c.qty, c.disc, c.total FROM tbCart AS c INNER JOIN tbProduct AS p ON c.pcode=p.pcode WHERE c.trasnno LIKE @trasnno and c.status LIKE 'Pending'", cn);
        ...
        public void GetTranNo()
        {
            try
            {
                ...
                string trasnno;
                cn.Open();
                cm = new SqlCommand("SELECT TOP 1 trasnno FROM tbCart WHERE trasnno LIKE '" + sdate + "%' ORDER BY id desc", cn);
                dr = cm.ExecuteReader();
                dr.Read();
                if (dr.HasRows)
                {
                    trasnno = dr[0].ToString();
                    count = int.Parse(trasnno.Substring(8, 4));
                    lblTranNo.Text = sdate + (count + 1);
                }
                else
                {
                    trasnno = sdate + "1001";
                    lblTranNo.Text = trasnno;
                }
                dr.Close();
                cn.Close();
            }
        ...
        public void AddToCart(string _pcode, double _price, int _qty)
        {
            try
            {
                ...
                else
                {
                    ...
                    cm.Parameters.AddWithValue("@cashir", lblUsername.Text);
                    cm.ExecuteNonQuery();
                    cn.Close();
                    LoadCart();
                }
        ...
        private void dgvCash_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "colAdd")
            {
                ...
                {
                    dbcon.ExecuteQuery("UPDATE tbCart SET qty = qty + " + int.Parse(txtQty.Text) + " WHERE trasnno LIKE '" + lblTranNo.Text + "'  AND pcode LIKE '" + dgvCash.Rows[e.RowIndex].Cells[2].Value.ToString() + "'");
                    LoadCart();
                }
                else
                {
