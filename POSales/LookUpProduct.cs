...
namespace POSales
{
    public partial class LookUpProduct : Form
    {
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT p.pcode, p.barcode, p.pdesc, b.brand, c.category, p.price, p.qty FROM tbProduct AS p INNER JOIN tbBrand AS b ON b.id = p.bid INNER JOIN tbCategory AS c on c.id = p.cid", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
